...
class particle():
    def __init__(self, particle_num, client, x_train, y_train):
        ...
        self.global_worst_model = client

        # best score init
        self.local_best_score = 0.0
        self.global_best_score = 0.0
        self.global_worst_score = 0.0
    ...
    def train_particle(self):
        ...
        for index, layer in enumerate(step_weight):
            ...
            r = np.random.rand(*layer.shape) - 0.5 # -0.5, 0.5
            if   0<=rnd and rnd<self.Cg:
                sol_name = 'current + global'
                new_weight[index] = step_weight[index] + alpha*(self.global_best_model.get_weights()[index]-layer) - (1-alpha)*(self.global_worst_model.get_weights()[index]-layer)  + r*mu 
            elif self.Cg<=rnd and rnd<self.Cp: 
                sol_name = 'current + personal'
                new_weight[index] = step_weight[index] + alpha*(self.local_best_model.get_weights()[index]-layer) - (1-alpha)*(self.global_worst_model.get_weights()[index]-layer) + r*mu
            elif self.Cp<=rnd and rnd<self.Cw:
                sol_name = 'globel + personal'
                new_weight[index] = beta_1*(self.global_best_model.get_weights()[index]) + beta_2*(self.local_best_model.get_weights()[index]) - beta_3*(self.local_best_model.get_weights()[index])+ r*mu
            else:
                sol_name='current weight + global + personal'
                new_weight[index] = step_weight[index] + beta_1*(self.global_best_model.get_weights()[index] - layer) + beta_2*(self.local_best_model.get_weights()[index]-layer) - beta_3*(self.global_worst_model.get_weights()[index]-layer) + r*mu
    ...
    def update_global_model(self, global_best_model, global_best_score, global_worst_model, global_worst_score):
        ...
        if global_worst_score < self.global_worst_score:    
            self.global_worst_model = global_worst_model
...
def find_gworst(server_result):
    # [pid, train_score, ]
    sorted_list_new = sorted(server_result, key=lambda x: x[1])
    print(sorted_list_new)
    print(server_result)
    return sorted_list_new[0][0], sorted_list_new[0][1] 
    ...
    for r in range(ROUND):
        ...
        for client in sso_model:
            if r != 0:
                ...
                client.update_global_model(server_model, acc, global_worst_model, global_worst_score)
        ...
        g_worst_id, this_worst_score = find_gworst(server_result)
        global_worst_model = sso_model[g_worst_id].local_best_model
