    ...
    def train_particle(self):
        ...
        for index, layer in enumerate(step_weight):
            ...
            if   0<=rnd and rnd<self.Cg:
                ...
                new_weight[index] = step_weight[index] + beta*(self.global_best_model.get_weights()[index]-layer) + r*mu 
            elif self.Cg<=rnd and rnd<self.Cp: 
                sol_name = 'current + personal'
                new_weight[index] = step_weight[index] + beta*(self.local_best_model.get_weights()[index]-layer) + r*mu
            elif self.Cp<=rnd and rnd<self.Cw:
                sol_name = 'current + glabel + personal'
                new_weight[index] = step_weight[index] + beta*(self.global_best_model.get_weights()[index] - layer) + (1-beta)*(self.local_best_model.get_weights()[index]-layer) + r*mu
            else:
                sol_name='current weight + global best'
                new_weight[index] = beta*(self.global_best_model.get_weights()[index]) + (1-beta)*(self.local_best_model.get_weights()[index]) + r*mu
